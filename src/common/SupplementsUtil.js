...
export const getSupplementsByLevel = (level) => {
  ...
  if (level === 1) {
    levelSupplements = createKeyAndValues(supplements.beginner);
  } else if (level === 2) {
    levelSupplements = [
...
export const getSupplementsBasedOnFitnessAndGoal = ({fitnessLevel, goal}) => {
  ...
  levelSupplements.map((supp) => {
    if (supp.fatLoss && goal === 0) {
      supplementList.push(supp);
    } else if (supp.healthy && goal === 1) {
      supplementList.push(supp);
    } else if (supp.weightGain && goal === 2) {
      supplementList.push(supp);
    }
  });
...
export const getSupplementKeysBasedOnFitnessAndGoal = ({
  fitnessLevel,
  goal,
}) => {
  ...
  levelSupplements.map((supp) => {
    if (supp.value.fatLoss && goal === 0) {
      supplementKeyList.push(supp.key);
    } else if (supp.value.healthy && goal === 1) {
      supplementKeyList.push(supp.key);
    } else if (supp.value.weightGain && goal === 2) {
      supplementKeyList.push(supp.key);
    }
  });
