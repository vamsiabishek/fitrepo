const supplements = {
  beginner: {
    multivitamin: {
      name: 'Multi-Vitamin',
      desc: 'Vitamins & Minerals supplement',
      detailedDesc:
        'Completely safe and necessary as we cannot get total daily nutrition value from our food sources',
      brands: [
        {
          brandName: 'ON(Optimum Nutrition)',
          name: 'ON Gold Standard whey',
        },
      ],
      image: require('../../assets/images/supplements/MultiVitamin.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Morning after breakfast'],
    },
    vitaminD: {
      name: 'Vit D',
      desc: 'Vitamin D supplement',
      detailedDesc:
        'Extremely necessary as it helps in production your body hormones',
      brands: [
        {
          name: 'D-rise',
          desc: 'Best buy from local pharmacy',
        },
      ],
      image: require('../../assets/images/supplements/VitaminD.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Night before bed'],
    },
    vitaminC: {
      name: 'Vit C',
      desc: 'Vitamin C supplement',
      detailedDesc:
        'Improves your immunity, protects you from getting sick frequently',
      brands: [
        {
          name: 'LimC',
          desc: 'Best buy from local pharmacy',
        },
      ],
      image: require('../../assets/images/supplements/VitaminC.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Anytime'],
    },
  },
  intermediate: {
    wheyConcentrate: {
      name: 'Whey',
      desc: 'Protein supplement',
      detailedDesc: 'Completely safe as it is made out of milk',
      brands: [
        {
          brandName: 'ON(Optimum Nutrition)',
          name: 'ON Gold Standard whey',
        },
      ],
      image: require('../../assets/images/supplements/WheyProtein.png'),
      bestConsume: ['Post workout'],
      fatLoss: true,
      weightGain: true,
      healthy: true,
      fitnessLevel: [2, 3],
    },
    fishOil: {
      name: 'Fish Oil',
      desc: 'Omega 3 supplement',
      detailedDesc: '',
      brands: [
        {
          brandName: 'ON(Optimum Nutrition)',
          name: 'Fish oil',
          desc: '',
        },
        {
          brandName: 'NOW',
          name: 'Omega 3',
          desc: '',
        },
      ],
      image: require('../../assets/images/supplements/FishOil.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Pre workout', 'Night before bed'],
    },
    lCarnitine: {
      name: 'L-carnitine',
      desc: 'Fat loss supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/L-Carnitine.png'),
      fatLoss: true,
      bestConsume: ['Before morning cardio', 'Pre workout'],
    },
    cla: {
      name: 'CLA',
      desc: 'Omega 6 supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/CLA.png'),
      fatLoss: true,
      bestConsume: ['Before morning cardio', 'Pre workout'],
    },
    bcaa: {
      name: 'BCAA',
      desc: 'Branch Chained Amino-Acid supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/BCAA.png'),
      fatLoss: true,
      weightGain: true,
      bestConsume: ['During morning cardio', 'Intra workout'],
    },
    creatine: {
      name: 'Creatine',
      desc: 'Creatine supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Creatine.png'),
      weightGain: true,
      bestConsume: ['Pre workout'],
    },
    fatBurner: {
      name: 'Fat Burner',
      desc: 'Fat burner supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/FatBurner.png'),
      weightGain: true,
      bestConsume: ['Before morning cardio', 'Pre workout'],
    },
    biotin: {
      name: 'Biotin',
      desc: 'Biotin supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Biotin.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Anytime'],
    },
    casein: {
      name: 'Casein',
      desc: 'Protein supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Casein.png'),
      weightGain: true,
      bestConsume: ['Night before bed'],
    },
  },
  advanced: {
    hmb: {
      name: 'HMB',
      desc: 'HMB supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/HMB.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      optional: true,
      bestConsume: ['Follow instructions on bottle'],
    },
    lGlutamine: {
      name: 'L-Glutamine',
      desc: 'Glutamine supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Glutamine.png'),
      fatLoss: true,
      weightGain: true,
      bestConsume: ['Post workout'],
    },
    glucosamine: {
      name: 'Glucosamine',
      desc: 'Joint support supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Glucosamine.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      optional: true,
      bestConsume: ['Anytime'],
    },
    ashwagandha: {
      name: 'Ashwagandha',
      desc: 'Stress releaving supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Ashwagandha.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      optional: true,
      bestConsume: ['Night before bed'],
    },
    tribulus: {
      name: 'Tribulus gokshura',
      desc: 'Testosterone booster supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Tribulus.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      optional: true,
      bestConsume: ['Night before bed'],
    },
    coq10: {
      name: 'CoQ10',
      desc: 'CoQ10 supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/CoQ10.png'),
      fatLoss: true,
      weightGain: true,
      healthy: true,
      bestConsume: ['Morning after breakfast'],
    },
    ubiquinol: {
      name: 'Ubiquinol',
      desc: 'Ubiquinol supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Ubiquinol.png'),
      fatLoss: true,
      weightGain: true,
      optional: true,
      alternativeTo: 'coq10',
      bestConsume: ['Morning after breakfast'],
    },
    zma: {
      name: 'ZMA',
      desc: 'Zinc, Magnesium and B6 supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/ZMA.png'),
      fatLoss: true,
      weightGain: true,
      bestConsume: ['Night before bed'],
    },
    betaAlanine: {
      name: 'Beta Alanine',
      desc: 'Beta Alanine supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/Beta-Alanine.png'),
      weightGain: true,
      bestConsume: ['Pre workout'],
    },
    lArginine: {
      name: 'L-Arginine',
      desc: 'L Arginine supplement',
      detailedDesc: '',
      brands: [],
      image: require('../../assets/images/supplements/L-Arginine.png'),
      weightGain: true,
      bestConsume: ['Pre workout'],
    },
  },
};

export default supplements;
